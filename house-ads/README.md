...
- アイコンの URL は **この JSON と同じホスト（qkserve.github.io）の https** だけが取得される（別ホストは無視）。`id` はバンドル ID の文字集合（英数字 `.` `_` `-`）のみ、`apps` は 50 件まで
- 壊れた要素は個別に無視され、全体が読めないときは端末の前回のキャッシュが使われる
- 一時的な設定（実機確認用の `perApp` の確率 1.0 など）は確認が済んだら外す（2026-09-17 の実機確認後に外し、既定 0.2 のみ）
